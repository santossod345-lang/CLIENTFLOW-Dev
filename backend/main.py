...
from backend.services import get_current_empresa
...
def ia_perguntar(
    ...
    try:
        if token:
            ...
            tenant_db.execute(f"SET search_path TO empresa_{empresa.id}, public")
        clientes = tenant_db.query(models.Cliente).filter(models.Cliente.empresa_id == empresa.id).all()
