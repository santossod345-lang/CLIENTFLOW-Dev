...
export default function PainelPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error('[Painel] Erro ao carregar dashboard:', err)
      // AxiosBridge já trata 401 com refresh automático.
      // Só mostramos a mensagem de erro se não for 401.
      if (err.response?.status !== 401) {
        const errorDetail = err.response?.data?.detail || 'Falha ao carregar dashboard'
